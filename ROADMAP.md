...
> 企业中枢的骨架：多部门协作 + 总裁办分拣 + 实时看板 + 完整工作流。

- [x] 多部门 Agent 架构（总裁办 + PMO·风控部·调度中心 + 财务部·公关部·运维部·QA部·研发部 + HR部 + 晨会播报官）
- [x] 总裁办分拣层 —— 自动识别闲聊/指令，闲聊直接回复，指令提炼标题后转PMO
- [x] 严格权限矩阵 —— 谁能给谁发消息，白纸黑字
- [x] 总控台实时看板（10 个功能面板）
- [x] 任务全生命周期管理（创建 → 分拣 → 规划 → 审议 → 派发 → 执行 → 汇报）
- [x] 任务叫停 / 取消 / 恢复
- [x] 结案报告系统（已完成需求单自动归档 + 五阶段时间线）
- [x] 任务模板库（9 个预设模板 + 参数表单 + 预估时间/费用）
- [x] 晨会动画感（每日首次打开播放开场动画 + 今日统计）
- [x] 行业早报（每日自动采集科技/财经资讯 + 飞书推送 + 订阅管理）
- [x] 模型热切换（看板内一键切换每个 Agent 的 LLM）
- [x] 技能管理（查看各省部已装 Skills + 添加新技能）
- [x] 高管总览（Token 消耗排行 + 活跃度 + 完成数统计）
- [x] 小任务 / 会话监控（OC-* 会话实时跟踪）
- [x] 需求单数据清洗 —— 标题/备注自动净化，脏数据拒绝入库
- [x] 重复任务防护 —— 已完成/已取消需求单不可覆盖
...
- [ ] 看板内实时连线动画：中书→门下→尚书→各部门
- [ ] 消息类型着色（派发 / 审议 / 汇报 / 打回重做）
- [ ] 时间线回放模式
- **难度**：⭐⭐⭐
...
### 📈 年度报告
- [ ] Agent 年度绩效报告（Token 总消耗、完成率、最复杂需求单）
- [ ] 可视化年度复盘大屏
- **难度**：⭐⭐
