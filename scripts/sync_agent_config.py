...
ID_LABEL = {
    'taizi':    {'label': '总裁办', 'role': '总裁办主任', 'duty': '飞书消息分拣与回复', 'emoji': '🏢'},
    'main':     {'label': '总裁办', 'role': '总裁办主任', 'duty': '飞书消息分拣与回复', 'emoji': '🏢'},  # 兼容旧配置
    'zhongshu': {'label': 'PMO',    'role': 'PMO总监',    'duty': '需求规划与优先级',   'emoji': '📊'},
    'menxia':   {'label': '风控部', 'role': '风控总监',   'duty': '审核与风控机制',     'emoji': '🛡️'},
    'shangshu': {'label': '调度中心','role': '运营总监',  'duty': '派单与升级裁决',     'emoji': '🔀'},
    'libu':     {'label': '公关部', 'role': '公关总监',  'duty': '文档/汇报/规范',     'emoji': '📝'},
    'hubu':     {'label': '财务部', 'role': '财务总监',  'duty': '资源/预算/成本',     'emoji': '📈'},
    'bingbu':   {'label': '运维部', 'role': '运维总监',  'duty': '应急与巡检',         'emoji': '🛡️'},
    'xingbu':   {'label': 'QA部',   'role': 'QA总监',    'duty': '合规/审计/红线',     'emoji': '🧪'},
    'gongbu':   {'label': '研发部', 'role': '研发总监',  'duty': '工程交付与自动化',   'emoji': '💻'},
    'libu_hr':  {'label': 'HR部',   'role': 'HR总监',    'duty': '人事/培训/Agent管理', 'emoji': '👥'},
    'zaochao':  {'label': '研究员', 'role': '行业研究员','duty': '每日新闻采集与简报', 'emoji': '📰'},
}
...
def deploy_soul_files():
    ...
    for proj_name, runtime_id in _SOUL_DEPLOY_MAP.items():
        ...
        if src_text != dst_text:
            ...
            deployed += 1
        # 总裁办兼容：同步一份到 legacy main agent 目录
        if runtime_id == 'taizi':
