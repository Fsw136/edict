...
用法:
  # 新建任务（收旨时）
  python3 kanban_update.py create REQ-20260223-012 "任务标题" Zhongshu PMO 中书令

  # 更新状态
  python3 kanban_update.py state REQ-20260223-012 Menxia "规划方案已提交风控部"

  # 添加流转记录
  python3 kanban_update.py flow REQ-20260223-012 "PMO" "风控部" "规划方案提交审核"

  # 完成任务
  python3 kanban_update.py done REQ-20260223-012 "/path/to/output" "任务完成摘要"

  # 添加/更新子任务 todo
  python3 kanban_update.py todo REQ-20260223-012 1 "实现API接口" in-progress
  python3 kanban_update.py todo REQ-20260223-012 1 "" completed

  # 🔥 实时进展汇报（Agent 主动调用，频率不限）
  python3 kanban_update.py progress REQ-20260223-012 "正在分析需求，拟定3个子方案" "1.调研技术选型|2.撰写设计文档|3.实现原型"
"""
...
STATE_ORG_MAP = {
    'Taizi': '总裁办', 'Zhongshu': 'PMO', 'Menxia': '风控部', 'Assigned': '调度中心',
...
_AGENT_LABELS = {
    'main': '总裁办', 'taizi': '总裁办',
    'zhongshu': 'PMO', 'menxia': '风控部', 'shangshu': '调度中心',
    'libu': '公关部', 'hubu': '财务部', 'bingbu': '运维部', 'xingbu': 'QA部',
    'gongbu': '研发部', 'libu_hr': 'HR部', 'zaochao': '研究员',
}
...
def _sanitize_text(raw, max_len=80):
    """清洗文本：剥离文件路径、URL、Conversation 元数据、旧前缀、截断过长内容。"""
    t = (raw or '').strip()
...
def cmd_create(task_id, title, state, org, official, remark=None):
    ...
    clean_remark = _sanitize_remark(remark) if remark else f"创建需求：{title}"
    def modifier(tasks):
...
_VALID_TRANSITIONS = {
    ...
    'Menxia':    {'Assigned', 'Zhongshu', 'Cancelled'},   # 打回可回PMO
...
def cmd_done(task_id, output_path='', summary=''):
    ...
    def modifier(tasks):
        ...
        t.setdefault('flow_log', []).append({
            ...
            "to": "老板", "remark": f"✅ 完成：{summary or '任务已完成'}"
        })
        t['updatedAt'] = now_iso()
        return tasks
    atomic_json_update(TASKS_FILE, modifier, [])
