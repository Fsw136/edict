...
def detect_official(agent_id):
    mapping = {
        'main':    ('高管', '总裁办'),        # legacy id for taizi
        'taizi':   ('高管', '总裁办'),
        'zhongshu': ('PMO总监', 'PMO'),
        'menxia':  ('风控总监', '风控部'),
        'shangshu': ('运营总监', '调度中心'),
    ...
    return mapping.get(agent_id, ('运营总监', '调度中心'))
...
def main():
    ...
    try:
        ...
        # merge manual parallel tasks (用于企业中枢并行看板展示)
        ...
        # ── 过滤掉非 REQ 且非活跃的系统会话，防止看板噪音 ──
        # 规则: 仅保留 24小时内更新的活跃会话，且排除 cron/subagent 等纯后台任务
        filtered_tasks = []
        one_day_ago = now_ms - 24 * 3600 * 1000
        for t in tasks:
            # 始终保留 REQ 任务（如果有的话，虽然这里主要是 OC 任务，但以防万一）
            if str(t['id']).startswith('REQ'):
        ...
        # ── 保留已有的 REQ-* 需求任务（不覆盖老板建单记录）──
        # REQ 任务的 now 字段由 Agent 自己通过 kanban_update.py progress 命令主动上报，
        ...
        if existing_tasks_file.exists():
            try:
                ...
                jjc_existing = [t for t in existing if str(t.get('id', '')).startswith('REQ')]
                
                # 去掉 tasks 里已有的 REQ（以防重复），再把需求放到最前面
                tasks = [t for t in tasks if not str(t.get('id', '')).startswith('REQ')]
                tasks = jjc_existing + tasks
            except Exception as e:
                log.error(f'merge existing REQ tasks failed: {e}')
                pass
