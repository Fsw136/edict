...
<p align="center">
  <strong>I modeled an AI multi-agent system after China's 1,300-year-old corporate governance.<br>Turns out, modern corporate design understood separation of powers better than modern AI frameworks.</strong>
</p>

<p align="center">
  <sub>12 AI agents (11 business roles + 1 compatibility role) form the Corporate Hub: GM Office triages, Planning proposes, Review vetoes, Dispatch assigns, Departments execute.<br>Built-in <b>institutional review gates</b> that CrewAI doesn't have. A <b>real-time dashboard</b> that AutoGen doesn't have.</sub>
</p>
...
<p align="center">
  ...
  <br>
  <sub>🎥 Full demo: AI Multi-Agent collaboration with Corporate Hub</sub>
</p>
...
<p align="center">
  ...
  <br>
  <sub>Submit request → GM Office triage → Planning → Review → Departments execute → Report back (30s)</sub>
</p>
</details>

> 🐳 **No OpenClaw?** Run `docker run -p 7891:7891 Fsw136/edict-demo` to try the full dashboard with simulated data.
...
```
You (CEO) → GM Office (Triage) → PMO → Risk Dept → Dispatch Center → Departments → Report Back
   老板              总裁办               PMO          风控部         调度中心           各部门          汇报
```

This isn't a cute metaphor. It's **real separation of powers** for AI:

- **GM Office (总裁办)** triages messages — casual chat gets auto-replied, real commands become tasks
- **Planning (PMO)** breaks your command into actionable sub-tasks
- **Review (风控部)** audits the plan — can reject and force re-planning
- **Dispatch (调度中心)** assigns approved tasks to specialist ministries
- **7 Departments** execute in parallel, each with distinct expertise
...
Edict's **Review Department (风控部)** exists specifically for this:
...
This is why Edict produces reliable results on complex tasks: there's a mandatory quality gate before anything reaches execution. Modern corporate governance teaches us — **unchecked power inevitably produces errors**.
...
- **GM Office** (总裁办) message triage — auto-reply casual chat, create tasks for real commands
- **Corporate Hub** (Planning · Review · Dispatch) for governance
- **Seven Departments** (Finance · Docs · Engineering · Compliance · Infrastructure · HR + Briefing) for execution
...
- Creates workspaces for all departments (`~/.openclaw/workspace-*`, including GM Office/HR/Briefing)
...
|------|----------|------|-----------|
| 👑 **GM Office** | `taizi` | Triage, summarize | Chat detection, intent extraction |
...
| From ↓ \ To → | Prince | Planning | Review | Dispatch | Departments |
|:---:|:---:|:---:|:---:|:---:|:---:|
| **GM Office** | — | ✅ | | | |
| **Planning** | ✅ | — | ✅ | ✅ | |
| **Review** | | ✅ | — | ✅ | |
...
```
CEO → Prince Triage → Planning → Review → Assigned → Executing → ✅ Done
                              ↑          │                       │
                              └── Veto ──┘              Blocked ──
```
...
│   ├── taizi/                  #   GM Office (triage)
│   ├── zhongshu/               #   PMO
│   ├── menxia/                 #   Risk Dept
│   ├── shangshu/               #   Dispatch Center
...
- [x] GM Office triage layer (chat vs task auto-routing)
...
<p align="center">
  ...
  <sub>以制度驾驭 AI，以协作创造价值</sub><br><br>
</p>
