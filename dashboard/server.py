    ...
    if archive_all_done:
        ...
        for t in tasks:
            if t.get('state') in ('Done', 'Cancelled') and not t.get('archived'):
                ...
                count += 1
        save_tasks(tasks)
        return {'ok': True, 'message': f'{count} 个需求已归档', 'count': count}
    task = next((t for t in tasks if t.get('id') == task_id), None)
    if not task:
        return {'ok': False, 'error': f'任务 {task_id} 不存在'}
...
def handle_create_task(title, org='PMO', official='PMO总监', priority='normal', template_id='', params=None, target_dept=''):
    """从看板创建新任务（需求模板建单）。"""
    ...
    # 清理常见前缀
    title = re.sub(r'^(建单)[：:\uff1a]\s*', '', title)
    if len(title) > 100:
        title = title[:100] + '…'
    # 标题质量校验：防止闲聊被误建为需求
    if len(title) < _MIN_TITLE_LEN:
        return {'ok': False, 'error': f'标题过短（{len(title)}<{_MIN_TITLE_LEN}字），不像是需求'}
    if title.lower() in _JUNK_TITLES:
        return {'ok': False, 'error': f'「{title}」不是有效需求，请输入具体工作指令'}
    # 生成 task id: REQ-YYYYMMDD-NNN
    today = datetime.datetime.now().strftime('%Y%m%d')
    tasks = load_tasks()
    today_ids = [t['id'] for t in tasks if t.get('id', '').startswith(f'REQ-{today}-')]
    seq = 1
    if today_ids:
        nums = [int(tid.split('-')[-1]) for tid in today_ids if tid.split('-')[-1].isdigit()]
        seq = max(nums) + 1 if nums else 1
    task_id = f'REQ-{today}-{seq:03d}'
    # 正确流程起点：老板 -> 总裁办分拣
    ...
    return {'ok': True, 'taskId': task_id, 'message': f'需求 {task_id} 已创建，正在派发给总裁办'}
...
def handle_review_action(task_id, action, comment=''):
    ...
    if action == 'approve':
        if task['state'] == 'Menxia':
            task['state'] = 'Assigned'
            task['now'] = '风控部审批通过，移交调度中心派发'
            remark = f'✅ 审批通过：{comment or "风控部审核通过"}'
            to_dept = '调度中心'
        else:  # Review
            task['state'] = 'Done'
            task['now'] = '审批通过，任务完成'
        ...
        task['state'] = 'Zhongshu'
        task['now'] = f'打回退回PMO修订（第{round_num}轮）'
        remark = f'🚫 打回重做：{comment or "需要修改"}'
        to_dept = 'PMO'
    else:
    ...
    task.setdefault('flow_log', []).append({
        'at': now_iso(),
        'from': '风控部' if task.get('state') != 'Done' else '老板',
        'to': to_dept,
        'remark': remark
    })
...
_AGENT_DEPTS = [
    {'id':'taizi',   'label':'总裁办',  'emoji':'🤴', 'role':'总裁办主任',     'rank':'高管'},
    {'id':'zhongshu','label':'PMO','emoji':'📜', 'role':'PMO总监',   'rank':'总监'},
    {'id':'menxia',  'label':'风控部','emoji':'🔍', 'role':'风控总监',     'rank':'总监'},
    {'id':'shangshu','label':'调度中心','emoji':'📮', 'role':'运营总监',   'rank':'总监'},
    {'id':'hubu',    'label':'财务部',  'emoji':'💰', 'role':'财务总监', 'rank':'部门长'},
    {'id':'libu',    'label':'公关部',  'emoji':'📝', 'role':'公关总监', 'rank':'部门长'},
    {'id':'bingbu',  'label':'运维部',  'emoji':'⚔️', 'role':'运维总监', 'rank':'部门长'},
    {'id':'xingbu',  'label':'QA部',  'emoji':'⚖️', 'role':'QA总监', 'rank':'部门长'},
    {'id':'gongbu',  'label':'研发部',  'emoji':'🔧', 'role':'研发总监', 'rank':'部门长'},
    {'id':'libu_hr', 'label':'HR部',  'emoji':'👔', 'role':'HR总监', 'rank':'部门长'},
    {'id':'zaochao', 'label':'研究员','emoji':'📰', 'role':'行业研究员',   'rank':'专家'},
]
...
_STATE_AGENT_MAP = {
    ...
    'Pending': 'zhongshu', # 待处理，默认PMO
}
_ORG_AGENT_MAP = {
    '公关部': 'libu', '财务部': 'hubu', '运维部': 'bingbu',
    'QA部': 'xingbu', '研发部': 'gongbu', 'HR部': 'libu_hr',
    'PMO': 'zhongshu', '风控部': 'menxia', '调度中心': 'shangshu',
}
...
def handle_scheduler_escalate(task_id, reason=''):
    ...
    target_label = '风控部' if next_level == 1 else '调度中心'
...
def handle_repair_flow_order():
    ...
    for task in tasks:
        ...
        first = flow_log[0]
        if first.get('from') != '老板' or first.get('to') != 'PMO':
            continue

        first['to'] = '总裁办'
        remark = first.get('remark', '')
        if isinstance(remark, str) and remark.startswith('建单：'):
            first['remark'] = remark

        if task.get('state') == 'Zhongshu' and task.get('org') == 'PMO' and len(flow_log) == 1:
            task['state'] = 'Taizi'
            task['org'] = '总裁办'
            task['now'] = '等待总裁办接单分拣'
...
# 状态推进顺序（手动推进用）
_STATE_FLOW = {
    'Pending':  ('Taizi', '老板', '总裁办', '待处理需求转交总裁办分拣'),
    'Taizi':    ('Zhongshu', '总裁办', 'PMO', '总裁办分拣完毕，转PMO起草'),
    'Zhongshu': ('Menxia', 'PMO', '风控部', 'PMO方案提交风控部审议'),
...
_STATE_LABELS = {
    'Pending': '待处理', 'Taizi': '总裁办', 'Zhongshu': 'PMO', 'Menxia': '风控部',
    'Assigned': '调度中心', 'Next': '待执行', 'Doing': '执行中', 'Review': '审查', 'Done': '完成',
}
...
def dispatch_for_state(task_id, task, new_state, trigger='state-transition'):
    ...
    _msgs = {
        ...
        'zhongshu': (
            ...
            f'请立即起草执行方案，走完完整三省流程（中书起草→门下审议→尚书派发→各部门执行）。'
        ),
        'menxia': (
            f'📋 PMO方案提交审议\n'
            f'任务ID: {task_id}\n'
...
def main():
    parser = argparse.ArgumentParser(description='三省各部门看板服务器')
    ...
    log.info(f'三省各部门看板启动 → http://{args.host}:{args.port}')
    print(f'   按 Ctrl+C 停止')
