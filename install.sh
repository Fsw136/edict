#!/bin/bash
# ══════════════════════════════════════════════════════════════
# 企业中枢 · OpenClaw Multi-Agent System 一键安装脚本
# ══════════════════════════════════════════════════════════════
set -e
...
banner() {
  echo ""
  echo -e "${BLUE}╔══════════════════════════════════════════╗${NC}"
  echo -e "${BLUE}║  🏛️  企业中枢 · OpenClaw Multi-Agent    ║${NC}"
  echo -e "${BLUE}║       安装向导                            ║${NC}"
  echo -e "${BLUE}╚══════════════════════════════════════════╝${NC}"
  echo ""
}
...
register_agents() {
  ...
  # 备份配置
  cp "$OC_CFG" "$OC_CFG.bak.edict-$(date +%Y%m%d-%H%M%S)"
...
tasks = [
    {
        ...
        "ac": "系统正常运行",
        "flow_log": [
            {"at": "2024-01-01T00:00:00Z", "from": "老板", "to": "PMO", "remark": "提需求初始化企业中枢系统"},
            {"at": "2024-01-01T00:01:00Z", "from": "PMO", "to": "风控部", "remark": "规划方案提交审核"},
            {"at": "2024-01-01T00:02:00Z", "from": "风控部", "to": "调度中心", "remark": "✅ 审批通过"},
            {"at": "2024-01-01T00:03:00Z", "from": "调度中心", "to": "研发部", "remark": "派发：系统初始化"},
            {"at": "2024-01-01T00:04:00Z", "from": "研发部", "to": "调度中心", "remark": "✅ 完成"},
        ]
    }
]
import os
