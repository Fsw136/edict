...
class TaskState(str, enum.Enum):
    """任务状态枚举 — 映射企业协作流程。"""
    Taizi = "Taizi"           # 总裁办分拣
    Zhongshu = "Zhongshu"     # PMO规划
    Menxia = "Menxia"         # 风控部审核
    Assigned = "Assigned"     # 调度中心已将任务派发
    Next = "Next"             # 待执行
    Doing = "Doing"           # 各部门执行中
    Review = "Review"         # 审查汇总
    Done = "Done"             # 完成
...
STATE_TRANSITIONS = {
    ...
    TaskState.Menxia: {TaskState.Assigned, TaskState.Zhongshu, TaskState.Cancelled},  # 打回退回PMO
...
class Task(Base):
    ...
    id = Column(String(32), primary_key=True, comment="任务ID, e.g. REQ-20260301-001")
    ...
    org = Column(String(32), nullable=False, default="总裁办", comment="当前执行部门")
