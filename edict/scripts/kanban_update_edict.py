...
用法（与旧版 100% 兼容）:
  python3 kanban_update.py create REQ-20260223-012 "任务标题" Zhongshu PMO PMO总监
  ...
  python3 kanban_update.py progress REQ-20260223-012 "正在分析需求" "1.调研✅|2.文档🔄|3.原型"
"""
...
def cmd_create(task_id, title, state, org, official, remark=None):
    ...
    if _check_api():
        ...
        result = _api_post('/api/tasks', {
            'title': title,
            'description': remark or f'创建需求：{title}',
            'priority': '中',
            'assignee_org': org,
            'creator': official,
            'tags': [task_id],
