...
export default function TaskModal() {
  ...
  const doReview = async (action: string) => {
    const labels: Record<string, string> = { approve: '审批通过', reject: '打回重做' };
        ...
        <div className="modal-body">
          ...
          <div className="task-actions">
            ...
            {['Review', 'Menxia'].includes(task.state) && (
              <>
                <button className="btn-action" style={{ background: '#2ecc8a22', color: '#2ecc8a', border: '1px solid #2ecc8a44' }} onClick={() => doReview('approve')}>✅ 审批通过</button>
                <button className="btn-action" style={{ background: '#ff527022', color: '#ff5270', border: '1px solid #ff527044' }} onClick={() => doReview('reject')}>🚫 打回重做</button>
              </>
            )}
          ...
          <div className="sched-section">
            ...
            <div className="sched-grid">
              ...
              <div className="sched-kpi"><div className="k">升级级别</div><div className="v">{!sched?.escalationLevel ? '无' : sched.escalationLevel === 1 ? '风控部' : '调度中心'}</div></div>
...
function LiveActivitySection({
  ...
  const phaseColors: Record<string, string> = {
    '老板': '#eab308', '总裁办': '#f97316', 'PMO': '#3b82f6', '风控部': '#8b5cf6',
    '调度中心': '#10b981', '各部门': '#06b6d4', '公关部': '#ec4899', '财务部': '#f59e0b',
    '运维部': '#ef4444', 'QA部': '#6366f1', '研发部': '#14b8a6', 'HR部': '#d946ef',
  };
