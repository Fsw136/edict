      ...
      <div className="mem-list">
        {!mems.length ? (
          <div style={{ textAlign: 'center', padding: '60px 24px' }}>
            ...
            <div style={{ fontSize: 15, fontWeight: 700, color: 'var(--text)', marginBottom: 8 }}>暂无结案报告</div>
            <div style={{ fontSize: 12, color: 'var(--muted)', lineHeight: 1.6 }}>任务完成或取消后将自动生成结案报告</div>
          </div>
        ) : (
          mems.map((t) => {
            const fl = t.flow_log || [];
...
function MemorialDetailModal({
  ...
  for (const f of fl) {
    ...
    else if (f.to === '风控部' || f.from === '风控部') reviewLog.push(f);
    ...
    <div className="modal-bg open" onClick={onClose}>
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-body">
          ...
          {renderPhase('PMO规划', '📋', planLog)}
          {renderPhase('风控审核', '🛡️', reviewLog)}
          {renderPhase('部门执行', '⚙️', execLog)}
          {renderPhase('结案汇报', '📨', resultLog)}
