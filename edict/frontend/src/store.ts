...
export const PIPE = [
  { key: 'Inbox',    dept: '老板',     icon: '👔', action: '提需求' },
  { key: 'Taizi',    dept: '总裁办',   icon: '🏢', action: '分拣' },
  { key: 'Zhongshu', dept: 'PMO',      icon: '📊', action: '规划' },
  { key: 'Menxia',   dept: '风控部',   icon: '🛡️', action: '审核' },
  { key: 'Assigned', dept: '调度中心', icon: '🔀', action: '派发' },
  { key: 'Doing',    dept: '各部门',   icon: '⚙️', action: '执行' },
  { key: 'Review',   dept: '调度中心', icon: '🔎', action: '汇总' },
  { key: 'Done',     dept: '汇报',     icon: '✅', action: '完成' },
] as const;
...
export const DEPT_COLOR: Record<string, string> = {
  '总裁办': '#e8a040', 'PMO': '#a07aff', '风控部': '#6a9eff', '调度中心': '#6aef9a',
  '公关部': '#f5c842', '财务部': '#ff9a6a', '运维部': '#ff5270', 'QA部': '#cc4444',
  '研发部': '#44aaff', 'HR部': '#9b59b6', '老板': '#ffd700', '汇报': '#2ecc8a',
};

export const STATE_LABEL: Record<string, string> = {
  Inbox: '收件', Pending: '待处理', Taizi: '总裁办分拣', Zhongshu: 'PMO规划',
  Menxia: '风控审核', Assigned: '已派发', Doing: '执行中', Review: '待审查',
...
export function stateLabel(t: Task): string {
  ...
  if (t.state === 'Menxia' && r > 1) return `风控审核（第${r}轮）`;
  if (t.state === 'Zhongshu' && r > 0) return `PMO修订（第${r}轮）`;
  return STATE_LABEL[t.state] || t.state;
}

export function isEdict(t: Task): boolean {
  return /^REQ-/i.test(t.id || '');
}
...
export const TAB_DEFS: { key: TabKey; label: string; icon: string }[] = [
  ...
  { key: 'court',     label: '高管会议', icon: '💼' },
  { key: 'monitor',   label: '部门调度', icon: '🔌' },
  { key: 'officials', label: '员工总览', icon: '👔' },
  { key: 'models',    label: '模型配置', icon: '🤖' },
  { key: 'skills',    label: '技能配置', icon: '🎯' },
  { key: 'sessions',  label: '小任务',   icon: '💬' },
  { key: 'memorials', label: '结案报告', icon: '📁' },
  { key: 'templates', label: '需求模板', icon: '📋' },
  { key: 'morning',   label: '行业早报', icon: '🌅' },
];
...
export const DEPTS = [
  ...
  { id: 'menxia',   label: '风控部',   emoji: '🛡️', role: '风控总监',    rank: '总监' },
  ...
  { id: 'xingbu',   label: 'QA部',     emoji: '🧪', role: 'QA总监',      rank: '部门长' },
...
export const TEMPLATES: Template[] = [
  {
    ...
    depts: ['财务部', '公关部'], est: '~10分钟', cost: '¥0.5',
    params: [
  ...
  },
  {
    ...
    depts: ['运维部', '财务部', '公关部'], est: '~60分钟', cost: '¥5',
    params: [
