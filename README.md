<h1 align="center">⚔️ 企业中枢 · Edict</h1>

<p align="center">
  <strong>我用现代企业管理制度，重新设计了 AI 多 Agent 协作架构。<br>结果发现，成熟的组织管理比现代 AI 框架更懂分权制衡。</strong>
</p>

<p align="center">
  <sub>12 个 AI Agent（11 个业务角色 + 1 个兼容角色）组成企业中枢：总裁办分拣、PMO规划、风控部审核打回重做、调度中心派发、各部门+HR部并行执行。<br>比 CrewAI 多一层<b>制度性审核</b>，比 AutoGen 多一个<b>实时看板</b>。</sub>
</p>
...
<p align="center">
  ...
  <br>
  <sub>🎥 企业中枢 AI 多 Agent 协作全流程演示</sub>
</p>

<details>
<summary>📸 GIF 预览（加载更快）</summary>
<p align="center">
  <img src="docs/demo.gif" alt="企业中枢 Demo" width="100%">
  <br>
  <sub>飞书提需求 → 总裁办分拣 → PMO规划 → 风控部审核 → 各部门并行执行 → 结案报告回报（30 秒）</sub>
</p>
</details>

> 🐳 **没有 OpenClaw？** 跑一行 `docker run -p 7891:7891 Fsw136/edict-demo` 即可体验完整看板 Demo（预置模拟数据）。
...
**企业中枢的思路完全不同** —— 我们借鉴了成熟的企业管理制度：

```
你 (老板) → 总裁办 (分拣) → PMO (规划) → 风控部 (审议) → 调度中心 (派发) → 各部门 (执行) → 汇报
```

这不是花哨的 metaphor，这是**真正的分权制衡**：

| | CrewAI | MetaGPT | AutoGen | **企业中枢** |
|---|:---:|:---:|:---:|:---:|
| **审核机制** | ❌ 无 | ⚠️ 可选 | ⚠️ Human-in-loop | **✅ 风控部专职审核 · 可打回重做** |
| **实时看板** | ❌ | ❌ | ❌ | **✅ 总控台 Kanban + 时间线** |
| **任务干预** | ❌ | ❌ | ❌ | **✅ 叫停 / 取消 / 恢复** |
| **流转审计** | ⚠️ | ⚠️ | ❌ | **✅ 完整结案报告存档** |
| **Agent 健康监控** | ❌ | ❌ | ❌ | **✅ 心跳 + 活跃度检测** |
...
<details>
<summary><b>🔍 为什么「风控部审核」是杀手锏？（点击展开）</b></summary>
...
- 📋 **审查方案质量** —— PMO的规划是否完备？子任务拆解是否合理？
- 🚫 **打回重做不合格的产出** —— 不是 warning，是直接打回重做
- 🔄 **强制返工循环** —— 直到方案达标才放行

这不是可选的插件——**它是架构的一部分**。每一个需求单都必须经过风控部，没有例外。

这就是为什么企业中枢能处理复杂任务而结果可靠：因为在送到执行层之前，有一个强制的质量关卡。成熟的企业管理早就证明了——**不受制约的权力必然会出错**。
...
### 🏛️ 多部门 Agent 架构
- **总裁办** 消息分拣 —— 闲聊自动回复，需求单才建任务
- **三大管理中枢**（PMO·风控部·调度中心）负责规划、审议、派发
- **六大执行部门**（财务部·公关部·运维部·QA部·研发部·HR部 + 晨会播报官）负责专项执行
- 严格的权限矩阵 —— 谁能给谁发消息，白纸黑字
- **状态流转校验** —— kanban_update.py 强制合法转换路径，非法状态跳转被拒绝
- 每个 Agent 独立 Workspace · 独立 Skills · 独立模型
- **需求单数据清洗** —— 标题/备注自动剥离文件路径、元数据、无效前缀
...
**📜 结案报告阁 · Memorials**
- 已完成需求单自动归档为结案报告
- 五阶段时间线：需求单→PMO→风控部→各部门→汇报
- 一键复制为 Markdown
- 按状态筛选

</td><td>

📜 **需求模板库 · Template Library**
- 9 个预设任务模板
- 分类筛选 · 参数表单 · 预估时间和费用
- 预览需求单 → 一键提需求
...
### 需求单看板
![需求单看板](docs/screenshots/01-kanban-main.png)
...
### 高管总览
![高管总览](docs/screenshots/06-official-overview.png)

### 会话记录
![会话记录](docs/screenshots/07-sessions.png)

### 结案报告归档
![结案报告归档](docs/screenshots/08-memorials.png)

### 任务模板
![任务模板](docs/screenshots/09-templates.png)

### 行业早报
![行业早报](docs/screenshots/10-morning-briefing.png)

### 晨会动画
![晨会动画](docs/screenshots/11-ceremony.png)

### 高管会议
![高管会议](docs/screenshots/12-court-discussion.png)
...
```bash
docker run -p 7891:7891 Fsw136/edict-demo
```
...
```bash
docker run --platform linux/amd64 -p 7891:7891 Fsw136/edict-demo
```
...
```bash
git clone https://github.com/Fsw136/edict.git
cd edict
chmod +x install.sh && ./install.sh
```

安装脚本自动完成：
- ✅ 创建全量 Agent Workspace（含总裁办/HR部/晨会播报，兼容历史 main）
- ✅ 写入各部门 SOUL.md（角色人格 + 工作流规则 + 数据清洗规范）
...
<p align="center">
  <img src="docs/assets/architecture-diagram.png" alt="企业中枢架构图" width="680">
  <br>
  <sub>老板 → 总裁办分拣 → PMO规划 → 风控部审议 → 调度中心派发 → 各部门并行执行 → 结案报告</sub>
</p>
...
| 部门 | Agent ID | 职责 | 擅长领域 |
|------|----------|------|---------|
| � **总裁办** | `taizi` | 消息分拣、需求整理 | 闲聊识别、需求单提炼、标题概括 |
| 📜 **PMO** | `zhongshu` | 接单、规划、拆解 | 需求理解、任务分解、方案设计 |
| 🔍 **风控部** | `menxia` | 审议、把关、打回重做 | 质量评审、风险识别、标准把控 |
| 📮 **调度中心** | `shangshu` | 派发、协调、汇总 | 任务调度、进度跟踪、结果整合 |
| 💰 **财务部** | `hubu` | 数据、资源、核算 | 数据处理、报表生成、成本分析 |
| 📝 **公关部** | `libu` | 文档、规范、报告 | 技术文档、API 文档、规范制定 |
| ⚔️ **运维部** | `bingbu` | 代码、算法、巡检 | 功能开发、Bug 修复、代码审查 |
| ⚖️ **QA部** | `xingbu` | 安全、合规、审计 | 安全扫描、合规检查、红线管控 |
| 🔧 **研发部** | `gongbu` | CI/CD、部署、工具 | Docker 配置、流水线、自动化 |
| 📋 **HR部** | `libu_hr` | 人事、Agent 管理 | Agent 注册、权限维护、培训 |
| 🌅 **晨会播报官** | `zaochao` | 每日晨会播报、新闻聚合 | 定时播报、数据汇总 |

### 权限矩阵

> 不是想发就能发 —— 真正的分权制衡

| From ↓ \ To → | 总裁办 | PMO | 风控部 | 调度中心 | 财务部 | 公关部 | 运维部 | QA部 | 研发部 | HR部 |
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| **总裁办** | — | ✅ | | | | | | | | |
| **PMO** | ✅ | — | ✅ | ✅ | | | | | | |
| **风控部** | | ✅ | — | ✅ | | | | | | |
...
<p align="center">
  <img src="docs/assets/task-flow-diagram.png" alt="任务状态流转图" width="500">
</p>
...
│   ├── taizi/SOUL.md           # 总裁办 · 消息分拣（含需求单标题规范）
│   ├── zhongshu/SOUL.md        # PMO · 规划中枢
│   ├── menxia/SOUL.md          # 风控部 · 审议把关
│   ├── shangshu/SOUL.md        # 调度中心 · 调度大脑
│   ├── hubu/SOUL.md            # 财务部 · 数据资源
│   ├── libu/SOUL.md            # 公关部 · 文档规范
│   ├── bingbu/SOUL.md          # 运维部 · 工程实现
│   ├── xingbu/SOUL.md          # QA部 · 合规审计
│   ├── gongbu/SOUL.md          # 研发部 · 基础设施
│   ├── libu_hr/                # HR部 · 人事管理
│   └── zaochao/SOUL.md         # 晨会播报官 · 情报枢纽
├── dashboard/
│   ├── dashboard.html          # 总控台看板（单文件 · 零依赖 · ~2500 行）
│   ├── dist/                   # React 前端构建产物（Docker 镜像内包含，本地可选）
│   ├── court_discuss.py        # 高管会议（多高管 LLM 讨论引擎）
│   └── server.py               # API 服务器（Python 标准库 · 零依赖 · ~2300 行）
├── scripts/
│   ├── run_loop.sh             # 数据刷新循环（每 15 秒）
│   ├── kanban_update.py        # 看板 CLI（含需求单数据清洗 + 标题校验）
...
通过 Feishu / Telegram / Signal 给PMO发消息：
...
1. 📜 PMO接单，规划子任务分配方案
2. 🔍 风控部审核，通过 / 打回重做打回重规划
3. 📮 调度中心审批通过，派发给运维部 + 研发部 + 公关部
4. ⚔️ 各部并行执行，进度实时可见
5. 📮 调度中心汇总结果，汇报给你
...
```bash
# 从 GitHub 添加 code_review skill 到PMO
...
<details>
<summary><b>❌ 任务总超时 / 下属完成了但无法传回总裁办</b></summary>

**症状**：各部门或调度中心已完成任务，但总裁办收不到回报，最终超时。
...
### Phase 1 — 核心架构 ✅
- [x] 多部门 Agent 架构（总裁办 + 管理中枢 + 执行部门 + 晨会播报官）+ 权限矩阵
- [x] 总控台实时看板（10 个功能面板 + 实时活动面板）
- [x] 任务叫停 / 取消 / 恢复
- [x] 结案报告系统（自动归档 + 五阶段时间线）
- [x] 任务模板库（9 个预设 + 参数表单）
- [x] 晨会动画感动画
- [x] 行业早报 + 飞书推送 + 订阅管理
- [x] 模型热切换 + 技能管理 + 技能添加
- [x] 高管总览 + Token 消耗统计
- [x] 小任务 / 会话监控
- [x] 总裁办消息分拣（闲聊自动回复 / 需求单建任务）
- [x] 需求单数据清洗（路径/元数据/前缀自动剥离）
- [x] 重复任务防护 + 已完成任务保护
- [x] 端到端测试覆盖（17 个断言）
...
### Phase 2 — 制度深化 🚧
- [ ] 老板审批模式（人工审批 + 一键审批通过/打回重做）
- [ ] 绩效看板（Agent 绩效评分体系）
- [ ] 消息总线（Agent 间实时消息流可视化）
- [ ] 知识库（知识库检索 + 引用溯源）
...
| 案例 | 需求单 | 涉及部门 |
|------|------|----------|
| [竞品分析](examples/competitive-analysis.md) | "分析 CrewAI vs AutoGen vs LangGraph" | PMO→风控部→财务部+运维部+公关部 |
| [代码审查](examples/code-review.md) | "审查这段 FastAPI 代码的安全性" | PMO→风控部→运维部+QA部 |
| [周报生成](examples/weekly-report.md) | "生成本周工程团队周报" | PMO→风控部→财务部+公关部 |

每个案例包含：完整需求单 → PMO规划 → 风控部审核意见 → 各部执行结果 → 最终结案报告。
...
[![Star History Chart](https://api.star-history.com/svg?repos=Fsw136/edict&type=Date)](https://star-history.com/#Fsw136/edict&Date)
...
<p align="center">
  <strong>⚡ 以制度驾驭 AI，以协作创造价值</strong><br>
  <sub>Governing AI with structured enterprise collaboration</sub><br><br>
</p>
