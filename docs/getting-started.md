...
> 从零开始，5 分钟搭建你的企业中枢 AI 协同系统

---

## 第一步：安装 OpenClaw

企业中枢基于 [OpenClaw](https://openclaw.ai) 运行，请先安装：
...
在 OpenClaw 中配置消息渠道（Feishu / Telegram / Signal），将 `taizi`（总裁办）Agent 设为需求单入口。总裁办会自动分拣闲聊与指令，指令类消息提炼标题后转发PMO。
...
1. **📋 需求单看板** — 观察任务在各状态之间流转
2. **🔭 省部调度** — 查看各部门工作分布
3. **📜 结案报告阁** — 任务完成后自动归档为结案报告

任务流转路径：
```
收件 → 总裁办分拣 → 中书规划 → 门下审议 → 已派发 → 执行中 → 已完成
```
...
> 在需求单看板或任务详情中，点击 **⏸ 叫停** 或 **🚫 取消** 按钮

### 订阅行业早报

> 看板 → 📰 行业早报 → ⚙️ 订阅管理 → 选择分类 / 添加源 / 配飞书推送
...
这是最常见的问题。企业中枢有 11 个 Agent，每个都需要 API Key。
...
- [🏠 项目首页](https://github.com/Fsw136/edict)
- [📖 README](../README.md)
