# 企业中枢任务分发流转体系 · 业务与技术架构

> 本文档详细阐述「企业中枢」项目如何从**业务制度设计**到**代码实现细节**，完整处理复杂多Agent协作的任务分发与流转。这是一个**制度化的AI多Agent框架**，而非传统的自由讨论式协作系统。

**文档概览图**

```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
业务层：企业管理制度 (Enterprise Governance Model)
  ├─ 分权制衡：老板 → 总裁办 → 中书 → 门下 → 尚书 → 各部门
  ├─ 制度约束：不可越级、状态严格递进、门下必审议
  └─ 质量保障：可打回重做反工、实时可观测、紧急可干预
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
| 保障机制 | 实现细节 | 防护效果 |
|---------|---------|---------|
| **制度性审核** | 风控部必审议所有中书方案，不可跳过 | 防止Agent胡乱执行，确保方案具有可行性 |
...
[*] --> Pending: 老板提需求
Pending --> Taizi: 总裁办接单
Taizi --> Zhongshu: 总裁办转交中书
Zhongshu --> Menxia: 中书提交审议
Menxia --> Zhongshu: 门下打回重做(可多次)
Menxia --> Assigned: 门下审批通过
Assigned --> Doing: 尚书派发执行
Doing --> Review: 各部完成
Review --> Done: 老板审批通过
Review --> Menxia: 老板要求修改
Done --> [*]
Doing --> [*]: 手动取消
Review --> [*]: 业务终止
```
...
```
DAY 1:
  10:00 - 老板飞书："为企业中枢编写完整自动化测试方案"
          总裁办接单。state = Taizi, org = 总裁办
          自动派发 taizi agent → 处理此需求单
  
  10:30 - 总裁办分拣完毕。判定为「工作需求单」（非闲聊）
          建任务 REQ-20260228-E2E
          flow_log 记录："老板 → 总裁办：提需求"
          state: Taizi → Zhongshu, org: 总裁办 → PMO
          自动派发 zhongshu agent

DAY 2:
  09:00 - PMO接单。开始规划
          汇报进展："分析测试需求，拆解为单元/集成/E2E三层"
          progress_log 记录："PMO 张三：分需求"
          
  15:00 - PMO完成方案
          todos 快照：需求分析✅、方案设计✅、待审议🔄
          flow_log 记录："PMO → 风控部：方案提交审议"
          state: Zhongshu → Menxia, org: PMO → 风控部
          自动派发 menxia agent
...
DAY 3:
  ...
  14:00 - 风控部审核完毕
          ...
          行为：✅ 审批通过 (带修改建议)
          flow_log 记录："风控部 → 调度中心：✅ 审批通过通过（5条建议）"
          state: Menxia → Assigned, org: 风控部 → 调度中心
          OPTIONAL：PMO收到建议，主动优化方案
          自动派发 shangshu agent

DAY 4:
  10:00 - 调度中心接到审批通过
          分析："该测试方案应派给研发部+QA部+公关部协力完成"
          flow_log 记录："调度中心 → 各部门：派发执行（兵吏合作）"
          state: Assigned → Doing, org: 调度中心 → 运维部+QA部+公关部
          自动派发 bingbu/xingbu/libu 三个agent（并行）

DAY 4-5:
  (各部并行执行)
  - 运维部(bingbu)：实现 pytest + unittest 测试框架
  - QA部(xingbu)：编写测试覆盖所有关键函数
  - 公关部(libu)：整理测试文档和用例说明
  
  实时汇报（hourly progress）：
  - 运维部："✅ 已实现 16 个单元测试"
  - QA部："🔄 正在编写集成测试（8/12 完成）"
  - 公关部："等待运维部完成再写报告"

DAY 5:
  14:00 - 各部完成
          state: Doing → Review, org: 运维部 → 调度中心
          调度中心汇总："所有测试已完成，通过率 98.5%"
          转回PMO
...
DAY 3 [打回重做场景]：
  14:00 - 风控部审核完毕
          ...
          行为：🚫 打回重做
          review_round += 1
          flow_log 记录："风控部 → PMO：🚫 打回重做（需补充性能测试）"
          state: Menxia → Zhongshu  # 返回中书修改
          自动派发 zhongshu agent（重新规划）

DAY 3-4：
  16:00 - PMO收到打回重做通知（唤醒agent）
          分析改进意见，补充性能测试方案
          progress："已整合性能测试需求，修正方案如下..."
          flow_log 记录："PMO → 风控部：修订方案（第2轮审议）"
          state: Zhongshu → Menxia
          自动派发 menxia agent

  18:00 - 风控部重新审议
          判定："✅ 本次通过"
          flow_log 记录："风控部 → 调度中心：✅ 审批通过通过（第2轮）"
...
```json
{
  "id": "REQ-20260228-E2E",          // 任务全局唯一ID (REQ-日期-序号)
  "title": "为企业中枢编写完整自动化测试方案",
  "official": "PMO总监",              // 负责官职
  "org": "PMO",                   // 当前负责部门
...
| 契约 | 含义 | 违反后果 |
|------|------|---------|
| **不可越级** | 总裁办只能调中书，中书只能调门下/尚书，各部门不能对外调用 | 超权调用被拒绝，系统自动拦截 |
| **状态单向递进** | Pending → Taizi → Zhongshu → ... → Done，不能跳过或倒退 | 只能通过 review_action(reject) 返回上一步 |
| **门下必审** | 所有中书提出的方案都要风控部审核，无法跳过 | 中书不能直接转尚书，门下必入 |
| **一旦Done无改** | 任务进入Done/Cancelled后不能再修改状态 | 若需修改需要创建新任务或取消后重新建 |
| **task_id唯一性** | REQ-日期-序号 全局唯一，同一天同一任务不重复建 | 看板防重，自动去重 |
...
```python
_STATE_FLOW = {
    'Pending':  ('Taizi',   '老板',    '总裁办',    '待处理需求单转交总裁办分拣'),
    'Taizi':    ('Zhongshu','总裁办',    'PMO',  '总裁办分拣完毕，转PMO起草'),
    'Zhongshu': ('Menxia',  'PMO',  '风控部',  'PMO方案提交风控部审核'),
    'Menxia':   ('Assigned','风控部',  '调度中心',  '风控部审批通过，转调度中心派发'),
    'Assigned': ('Doing',   '调度中心',  '各部门',    '调度中心开始派发执行'),
    'Next':     ('Doing',   '调度中心',  '各部门',    '待执行任务开始执行'),
    'Doing':    ('Review',  '各部门',    '调度中心',  '各部完成，进入汇总'),
    'Review':   ('Done',    '调度中心',  '总裁办',    '全流程完成，汇报总裁办转报老板'),
}
```
...
2. 构造派发消息（针对性促使Agent立即工作）
   ├─ taizi: "📜 老板需求单需要你处理..."
   ├─ zhongshu: "📜 需求单已到PMO，请起草方案..."
   ├─ menxia: "📋 PMO方案提交审议..."
   ├─ shangshu: "📮 风控部已审批通过，请派发执行..."
   └─ 各部门: "📌 请处理任务..."
...
| 场景 | 请求 | 结果 | 理由 |
|------|------|------|------|
| **正常** | PMO → 风控部审核 | ✅ 允许 | 门下在中书的 allowAgents 中 |
| **违反** | PMO → 调度中心改方案 | ❌ 拒绝 | 中书只能调门下/尚书，不能手工改尚书工作 |
| **违反** | 研发部 → 调度中心 "我完成了" | ✅ 改状态 | 通过 flow_log 和 progress_log（不是跨Agent调用） |
| **违反** | 调度中心 → PMO "重新改方案" | ❌ 拒绝 | 尚书不在门下/中书的 allowAgents 中 |
...
单个任务的59条活动流（REQ-20260228-E2E 示例）：
...
T+180:
  ...
  ✅ 阶段1：重试
  - retryCount: 0 → 1
  - dispatch_for_state('REQ-20260228-E2E', 'Zhongshu', trigger='taizi-scan-retry')
  - 派发消息发送到PMO（唤醒agent或重启）
  - flow_log: "停滞180秒，自动重试第1次"

T+ 240:
  PMO Agent 恢复（或手工重启），收到重试派发
  汇报进展："已恢复，继续规划..."
...
T+360 (若仍未恢复):
  ...
  ✅ 阶段2：升级
  - escalationLevel: 0 → 1
  - wake_agent('menxia', "💬 任务REQ-20260228-E2E停滞，PMO无反应，请介入")
  - flow_log: "升级至风控部协调"
...
T+540 (若仍未解决):
  ...
  ✅ 阶段3：再次升级
  - escalationLevel: 1 → 2
  - wake_agent('shangshu', "💬 任务长期停滞，PMO+风控部都无法推进，调度中心请介入协调")
  - flow_log: "升级至调度中心协调"
...
T+720 (若仍未解决):
  ...
  - dispatch_for_state('REQ-20260228-E2E', 'Assigned', trigger='taizi-auto-rollback')
  - flow_log: "连续停滞，自动回滚到Assigned，由调度中心重新派发"
  
  结果：
  - 调度中心重新派发给各部门执行
  - PMO的方案保留在前一个 snapshot 版本中
  - 用户可以看到回滚操作，决定是否介入
```
...
请求：
{
  ...
  "params": {},
  "target_dept": "运维部+QA部"  // 可选，派发建议
}

响应：
{
  "ok": true,
  "taskId": "REQ-20260228-001",
  "message": "需求单 REQ-20260228-001 已下达，正在派发给总裁办"
}
```
...
```
请求：
GET /api/task-activity/REQ-20260228-E2E

响应：
{
  "ok": true,
  "taskId": "REQ-20260228-E2E",
  "taskMeta": {
    "title": "为企业中枢编写完整自动化测试方案",
    "state": "Assigned",
    "org": "调度中心",
...
{
  "ok": true,
  "message": "REQ-20260228-E2E 已推进到下一阶段 (已自动派发 Agent)",
  "oldState": "Zhongshu",
  "newState": "Menxia",
  "targetAgent": "menxia"
}
...
{
  "ok": true,
  "message": "REQ-20260228-E2E 已审批通过 (已自动派发 Agent)",
  "state": "Assigned",
  "reviewRound": 1
}
```
...
python3 scripts/kanban_update.py progress \
  REQ-20260228-E2E \
  "已完成需求分析和方案初稿，现正征询研发部意见" \
  "1.需求分析✅|2.方案设计✅|3.研发部咨询🔄|4.待门下审议"
...
#     "text": "已完成需求分析和方案初稿，现正征询研发部意见",
#     "state": task.state,
#     "org": task.org,
#     "todos": [
#       {"id": "1", "title": "需求分析", "status": "completed"},
#       {"id": "2", "title": "方案设计", "status": "completed"},
#       {"id": "3", "title": "研发部咨询", "status": "in-progress"},
...
### CrewAI / AutoGen 的传统方式 vs 企业中枢的制度化方式

| 维度 | CrewAI | AutoGen | **企业中枢** |
|------|--------|---------|----------|
| **协作模式** | 自由讨论（Agent自主选择协作对象） | 面板+回调（Human-in-the-loop） | **制度化协作（权限矩阵+状态机）** |
| **质量保障** | 依赖Agent智能（无审核）| Human审核（频繁中断） | **自动审核（风控部必审）+可干预** |
...
| **吞吐量** | 1任务1Agent（串行讨论） | 1任务1Team（需人工管理） | **多任务并行（各部门同时执行）** |
...
# 风控部想升级到老板
try:
    result = dispatch_to_agent('taizi', '我需要老板的指示')
except PermissionError:
    # ❌ 权限矩阵拦截
...
python3 scripts/kanban_update.py flow \
  ...
  "风控部" \
  "PMO" \
  "🚫 打回重做：协议部分过于简略，需补充权限矩阵示例"

# PMOAgent 收到唤醒，重新修改方案
# （3小时后 → 重新提交门下审议）
...
python3 scripts/kanban_update.py state \
  REQ-20260302-001 \
  Doing \
  "派发给公关部+运维部+研发部三部并行执行"

python3 scripts/kanban_update.py flow \
  REQ-20260302-001 \
  "调度中心" \
  "各部门" \
  "派发执行：公关部排版|运维部代码示例|研发部基础设施部分"
...
python3 scripts/kanban_update.py done \
  ...
  "企业中枢协议文档已完成，包含89页，5个阶段历时3天，总消耗成本$2.34"
...
curl http://127.0.0.1:7891/api/task-activity/REQ-20260302-001
...
**企业中枢是一个制度化的AI多Agent系统**，不是传统的"自由讨论"框架。它通过：

1. **业务层**：借鉴成熟的企业管理体系，建立分权制衡的组织结构
...
相比 CrewAI/AutoGen 的"自由+人工管理"，企业中枢提供了一套**企业级的AI协作框架**。
