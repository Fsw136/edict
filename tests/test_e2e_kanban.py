...
def _backup_and_restore():
    ...
    tasks = [t for t in tasks if not t.get('id', '').startswith('REQ-TEST-')]
    TASKS_FILE.write_text(json.dumps(tasks, ensure_ascii=False, indent=2))


# ── TEST 1: 脏标题(含文件路径+Conversation)应被清洗后创建
def test_dirty_title_cleaned():
    cmd_create('REQ-TEST-E2E-01',
        '全面审查/opt/edict/这个项目\nConversation info (xxx)',
        'Zhongshu', 'PMO', 'PMO总监',
        '创建需求：全面审查/opt/projects/项目')
    t = _get_task('REQ-TEST-E2E-01')
...
def test_pure_path_rejected():
    cmd_create('REQ-TEST-E2E-02', '/opt/edict/', 'Zhongshu', 'PMO', 'PMO总监')
    assert _get_task('REQ-TEST-E2E-02') is None, "纯路径标题应被拒绝"


# ── TEST 3: 正常标题正常创建
def test_normal_title():
    cmd_create('REQ-TEST-E2E-03', '调研工业数据分析大模型应用方案', 'Zhongshu', 'PMO', 'PMO总监', '总裁办整理需求')
    t = _get_task('REQ-TEST-E2E-03')
...
def test_flow_remark_cleaned():
    cmd_create('REQ-TEST-E2E-04', '调研工业数据分析大模型应用方案', 'Zhongshu', 'PMO', 'PMO总监')
    cmd_flow('REQ-TEST-E2E-04', '总裁办', 'PMO', '需求传达：审查/opt/projects/xxx项目 Conversation blah')
    t = _get_task('REQ-TEST-E2E-04')
...
def test_short_title_rejected():
    cmd_create('REQ-TEST-E2E-05', '好的', 'Zhongshu', 'PMO', 'PMO总监')
    assert _get_task('REQ-TEST-E2E-05') is None, "短标题应被拒绝"


# ── TEST 6: 建单前缀剥离
def test_prefix_stripped():
    cmd_create('REQ-TEST-E2E-06', '帮我写技术博客文章关于智能体架构', 'Zhongshu', 'PMO', 'PMO总监')
    t = _get_task('REQ-TEST-E2E-06')
    assert t is not None, "任务应被创建"
    assert not t['title'].startswith('建单'), f"前缀应被剥离: {t['title']}"


# ── TEST 7: state 更新 + org 自动联动
def test_state_update():
    cmd_create('REQ-TEST-E2E-07', '测试状态更新与组织联动功能', 'Zhongshu', 'PMO', 'PMO总监')
    cmd_state('REQ-TEST-E2E-07', 'Menxia', '方案提交风控部审核')
    t = _get_task('REQ-TEST-E2E-07')
    assert t is not None
    assert t['state'] == 'Menxia', f"state应为Menxia: {t['state']}"
    assert t['org'] == '风控部', f"org应为风控部: {t['org']}"


# ── TEST 8: done 完成
def test_done():
    cmd_create('REQ-TEST-E2E-08', '测试任务完成状态标记功能', 'Zhongshu', 'PMO', 'PMO总监')
    cmd_done('REQ-TEST-E2E-08', '/tmp/output.md', '任务已完成')
    t = _get_task('REQ-TEST-E2E-08')
    assert t is not None
...
def test_done_not_overwritable():
    cmd_create('REQ-TEST-E2E-09', '测试已完成任务不可覆盖保护', 'Zhongshu', 'PMO', 'PMO总监')
    cmd_done('REQ-TEST-E2E-09', '/tmp/output.md', '任务已完成')
    cmd_create('REQ-TEST-E2E-09', '试图覆盖已完成的任务标题', 'Zhongshu', 'PMO', 'PMO总监')
    t = _get_task('REQ-TEST-E2E-09')
    assert t is not None
