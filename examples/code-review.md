...
> **需求单**：审查一段 FastAPI 代码的安全性，输出问题清单和修复建议
...
| # | 子任务 | 派发部门 | 说明 |
|---|--------|----------|------|
| 1 | 代码安全扫描 | ⚔️ 运维部 | 逐文件审查，标记安全问题 |
| 2 | 合规与最佳实践检查 | ⚖️ QA部 | 对照 OWASP Top 10 检查合规性 |
| 3 | 修复方案输出 | ⚔️ 运维部 | 针对每个问题给出修复代码 |
...
**风控部批语：** *"方案合理。运维部负责工程侧审查，QA部负责合规侧审查，分工明确。注意QA部需出具 OWASP 逐项对照表，不能只写'基本合规'。审批通过。"*
...
**QA部结论：** 6/10 项需要处理，其中 2 项不合规（A02、A03），需立即修复。
...
风控部首轮即审批通过，运维部和QA部分工明确：运维部负责代码层面扫描和修复，QA部负责合规框架对照。
